...
// ACHTUNG: Ausgänge AA0 und AA1 müssen vertauscht werden!
void aufgabe_8_6()
{
    // Setzen der abzuarbeitenden Betriebsspannungen
    ...
    for (double const &betriebsspannung_Volt: betriebsspannungen_Volt)
    {
        ...
        std::string dateiname =
                "Verstaerkungskennlinie_ID_von_UGS_bei_UDS_von_" + std::to_string(betriebsspannung_Volt) + "V.csv";
        std::string dateipfad = orderpfad + dateiname;
...
void aufgabe_8_11()
{
    ...
    for (double const &spannungBasis_Volt: spannungenBasis_Volt)
    {
        ...
        for (uint16_t i = 0; i < buffer_AE1.size(); i++)
        {
            ...
            double stromstaerkeCollector_Ampere =
                    static_cast<double>(betriebsspannung_Volt - toVolt(buffer_AE1.at(i))) / R1_Ohm;
