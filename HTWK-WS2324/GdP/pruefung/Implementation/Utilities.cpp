...
void Utilities::printMitarbeiter(int mitarbeiterId, MitarbeiterDatenbank *db)
{
	Mitarbeiter mitarbeiter = db->getMitarbeiter(mitarbeiterId);
	db->selectedId = mitarbeiterId;
	printMitarbeiter(mitarbeiter, db);
}

void Utilities::printMitarbeiter(Mitarbeiter mitarbeiter, MitarbeiterDatenbank *db)
{
	try
	{
...
void Utilities::printMitarbeiterTable(std::map<int, Mitarbeiter> mitarbeiterMap)
{
	...
	std::cout << std::setw(maxIdWidth) << "ID" << std::setw(maxNameWidth) << "Name" << std::setw(maxVornameWidth) << "Vorname"
			  << std::setw(maxPLZWidth) << "PLZ" << std::setw(maxGehaltWidth) << "Gehalt (EUR)" << std::endl;
...
void Utilities::handleSalaryChange(Mitarbeiter &mCurrent, std::string &factorStr, MitarbeiterDatenbank *db)
{
	double factor;
	if (tryParse(factorStr, factor))
	{
		std::cout << COLOR_YELLOW << mCurrent.gehalt() << "€" << COLOR_WHITE << "  -->  " << COLOR_GREEN << (mCurrent.gehalt() * factor)
				  << "€" << std::endl << RESET_STYLE << std::endl;

		if (Utilities::askQuestion("Änderungen speichern?", true))
		{
			db->erhoeheGehalt(db->selectedId, factor);
